"""
AUTOMATION CIRCUIT CONTROL - GENERATOR WITH SWITCH - TEST 2.2, 2021
...
NOTE 2: THIS CODE SHOULD BE UNIQUELY RAN WITH THE TEST 2.2 ELECTRICAL SET UP. IF NOT, THERE'S A POSSIBILITY OF DAMAGING POWER METER OR GENERATOR.
"""
...
def init():
    ...
    # log.debug("Write to a register and read back reset=0")
...
"""
def run_sync_client(power, sleep_time, micro_time, freq, num_its, switch1, switch2):
    ...
    for i in range(num_its):
        ...
        # TURN MICROWAVES ON
        # Place switch in position I (A-B , ablation probe to power meter)
        rb.switchon(switch1)
        ...
        # TURN MICROWAVE OFF
        # Place switch in position II (A-D , ablation probe to 50 ohm termination resistance)
    ...
    # Place switch in position II again for redundency purposes
    client.write_register(2, 0xBF, unit=UNIT)
    rb.switchon(switch2)
    rb.switchoff(switch1)
...
# ------Create Definition ------ #
create_layout = [
    [sg.Text('Generator + switch (Test 2.2)', font=("Helvetica", 18), pad=(0, 5)),
...
window = sg.Window('GENERATOR + SWITCH TEST - Test 2.2 - IARC', layout, default_element_size=(40, 1), grab_anywhere=False,
                   size=(800, 400))
